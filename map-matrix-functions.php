...
function get_geo_info_from_postcode($request){
  $geocoding_info = fetch_postcodes_io_postcode_info($request);
  var_dump($geocoding_info);
...
function get_geo_info_from_lat_lng($request){
 ...
 // var_dump($geocoding_info);
  global $wp_session;
  if($geocoding_info->status== 200){
    foreach($geocoding_info->data->result as $key=>$result){
